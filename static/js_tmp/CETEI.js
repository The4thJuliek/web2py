    ...
    */
    domToHTML5(XML_dom, callback, perElementFn){
      ...
      let convertEl = (el) => {
        ...
        //-----------------------------------
        //--- XCC ALMA-Viewer functions
        //-----------------------------------
        newElement.setAttribute("id", "xcc_" + el.localName );
        newElement.setAttribute("class",  "_" + el.localName);

        //---- Probar afegir les sigles
        if( el.localName == "lem"){
            //console.log(el);
            //console.log(el.textContent);
           if( el.textContent == "" ){
            el.textContent = "";
            newElement.setAttribute("data-lem-empty", "1")
           }
        }

        //---  Un data atribut nou per a les notes
        //--- El necessitem per a mostrar el text al Info-Box i poder borrar el contingut
        //--- que substituirem amb el número de la nota
        if( el.localName == "witDetail" ){
          newElement.setAttribute("data-note-text", "*");
          newElement.setAttribute("data-note-number", "0");
        }

        //--- Sigle extraieren und als einzelne Element vorbereiten
        //--- Die CSS-Class xcc-sigla verwenden
        let tempValue;

        if( el.localName == "lem" || el.localName == "rdg"){
          //tempValue = el.attr("wit")
        }
        //--- ENDE Sigle extraieren


let noteText;        
        if( el.localName == "note" ){
//console.log(el); 
          noteText = el.innerHTML;
          newElement.setAttribute("data-note-text", noteText);
          //newElement.setAttribute("data-note-number", "0");
        }

        //-----------------------------------
        //--- END XCC ALMA-Viewer functions
        //-----------------------------------
